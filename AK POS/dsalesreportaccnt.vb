...
Public Class dsalesreportaccnt
    ...
    Private Sub btnok_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnok.Click
        Try
            ...
            dsalesprevaccnt.condition = " transdate>='" & Format(datefrom.Value, "MM/dd/yyyy") & "' and transdate<='" & Format(dateto.Value, "MM/dd/yyyy") & "' and refund='0'"

            If cmbcashier.SelectedItem <> "All" Then
                dsalesprevaccnt.condition = dsalesprevaccnt.condition & " and cashier='" & cmbcashier.SelectedItem & "'"
                dsalesprevaccnt.cashr = cmbcashier.SelectedItem
            End If

            If cmbservice.SelectedIndex <> 0 Then
                dsalesprevaccnt.condition = dsalesprevaccnt.condition & " and servicetype='" & cmbservice.SelectedItem & "'"
            End If

            If cmbdisc.SelectedIndex = 1 Then
                dsalesprevaccnt.condition = dsalesprevaccnt.condition & " and disctype<>'" & strnone & "'"
            ...
            If cmbtype.Visible = True Then
                If cmbtype.SelectedIndex <> 0 Then
                    dsalesprevaccnt.condition = dsalesprevaccnt.condition & " and disctype='" & cmbtype.SelectedItem & "'"
                End If
            End If
            ...
            sql = "Select * from tbltransaction where" & dsalesprevaccnt.condition
            connect()
            cmd = New SqlCommand(sql, conn)
            dr = cmd.ExecuteReader
            If dr.Read Then
                Me.Cursor = Cursors.WaitCursor
                If dsalesprevaccnt.ctr = True Then
                    dsalesprevaccnt.ShowDialog()
                    dsalesprevaccnt.WindowState = FormWindowState.Normal
                    dsalesprevaccnt.daterange = dfrom & "_TO_" & dto
                    dsalesprevaccnt.lbldate.Text = dfrom & "_TO_" & dto
                    Me.Cursor = Cursors.Default
                    Me.Close()
                    Exit Sub
                End If
                ' dsalesprevaccnt.MdiParent = mdiform
                dsalesprevaccnt.Show()
                dsalesprevaccnt.WindowState = FormWindowState.Maximized
                dsalesprevaccnt.daterange = dfrom & "_TO_" & dto
                dsalesprevaccnt.lbldate.Text = dfrom & "_TO_" & dto
                Me.Cursor = Cursors.Default
                Me.Close()
